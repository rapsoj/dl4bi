#!/usr/bin/env python3
import hydra
import jax.numpy as jnp
from jax import random
from omegaconf import DictConfig
from sps.kernels import matern_3_2
from uci import load_dataset

from dl4bi.core import gp_mle_sgd


@hydra.main("configs/uci", config_name="default", version_base=None)
def main(cfg: DictConfig):
    num_restarts, num_samples = 10, 2048
    rng_data, rng = random.split(random.key(cfg.seed))
    s_train, f_train, *_ = load_dataset(rng_data, cfg.data.name)
    thetas = []
    for _ in range(num_restarts):
        rng_idx, rng = random.split(rng)
        idx = random.choice(rng_idx, num_samples, (num_samples,), replace=False)
        var, ls, noise = gp_mle_sgd(s_train[idx], f_train[idx], matern_3_2)
        print(f"var {var:0.4f}, ls {ls:0.4f}, noise: {noise:0.4f}")
        thetas += [[var, ls, noise]]
    var_mu, ls_mu, noise_mu = jnp.array(thetas).mean(axis=0)
    print(f"Average: var {var_mu:0.4f}, ls {ls_mu:0.4f}, noise: {noise_mu:0.4f}")


if __name__ == "__main__":
    main()
